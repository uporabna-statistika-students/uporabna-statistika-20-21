...
# 2. Preučevanje rezultatov

Najprej si poglejmo celotne _trace plot_-e za $\mu_j$ kjer sem za $j$ izbral vrednosti $2, 8, 9$ in $42$.
...
Izgleda da smo dosegli konvergenco. Lepo je videti primerjavo med šolo $9$ in $42$, kako se konvergenca dogaja okrog različne vrednosti $\mu$. _Burn-in_ ni opazen v nobenem izmed izbranih primerov, kar je za pričakovati saj smo za začetno vrednost izbrali vzorčno povprečje posamezne skupine. Mogoče bo _Burn-in_ bolj opazen na grafih z manj členi zaporedja. Poglejmo si torej enake slike le s $500$ členi zaporedja.
...
Podobno kot prej, za izbrane $\mu_j$ vrednosti, ne vidimo _Burn-in_ dela. Za $\mu_j$ izgledajo verige v redu. Poglejmo si še podobne grafe za $\sigma_j^2$.
...
Podobno kot za $\mu_j$, zgleda da smo dosegli konvergenco. _Burn-in_ ravno tako ni opazen v nobenem izmed izbranih primerov in ravno tako kot pri $\mu_j$, smo tudi tu določili začetne vrednosti variance, kar vzorčne variance posameznih skupin. Največ kar opazimo je to da je varianca v skupinah $2$ in $8$ večja kot v skupinah $9$ in $42$ (vrednosti so nekoliko višje). Zopet si poglejmo če bomo opazili _burn-in_ pri prvih 500 členih zaporedja.
...
Zopet ne vidimo _burn-in_ dela tako da lahko rečemo da so tudi tu verige v redu. Poglejmo si podobne grafe še za hiperparametre.
...
Pri hiperparametrih ravno tako zgleda da smo dosegli konvergenco. Zopet ne vidimo _burn-in_ dela. Največ kar je opazno je diskretnost hiperparametra $\nu_0$. Ponovno si poglejmo še grafe prvih $500$ členov zaporedja.
...
Ponovno ni videti _burn-in_ dela verig, zato bomo ohranili celotne verige.

Poglejmo si še "stabilnost" naših verig. Verige razdelimo na $10$ enako velikih delov. Podvzorec določimo kot $\frac{1}{10}$ zaporednih členov celotne verige. Za tem primerjamo njihove porazdelitve. Najprej si poglejmo slike za $\mu_j$.
...
Podobno kot prej, vidimo razlike med posameznimi skupinami, medtem ko so podvzorci znotraj skupine "stabilni". Poglejmo si podobno sliko še za $\sigma_j^2$.
...
Ravno tako opazimo razlike med posameznimi skupinami. Če mogoče ni bilo opazno na _trace plot_-ih je tu precej bolje videti da so porazdelitve nekoliko asimetrične. To je tudi smiselno saj jih vzorčimo iz inverzne gamma porazdelitve, ki je asimetrična. Poglejmo si še podvzorce za hiperparametre.
...
Kar se "stabilnosti" tiče ravno tako ne vidimo težav. Na prvi pogled izgleda, da bi lahko težavo predstavljali podvzorci za $\nu_0$, vendar se "poskakovanje" mediane dogaja zaradi diskretnosti.

Poglejmo si še prikaze avtokorelacij za vse naše (hiper)parametre.
...
Večjih težav z avtokorelacijo pri $\mu_j$ in $\sigma_j^2$, ni videti. Pri hiperparametrih vidimo da gre pri vseh parametrih korelacija z manjšo zakasnitvijo k $0$. Smiselno si je pogledati še _effective sample size_ vseh parametrov. Zraven je dodana še "standardna napaka" (MCSE - Monte Carlo Standard Error).
...
Vidimo da je _effective sample size_ višji pri že vidno boljših prikazih avtokorelacije. Pri hiperparametrih kjer smo že videli težavo je tudi _effective sample size_ nižji. Velikost _MCSE_ je smiselno interpretirati v sklopu dejanskih vrednosti (hiper)parametrov, kar si bomo pogledali v naslednjem poglavju.

# 3. Robne aposteriorne porazdelitve

Poglejmo si torej najprej robne aposteriorne porazdelitve za $\mu_j$ in $\sigma_j^2$.

```{r, echo=FALSE, eval=TRUE, results="markup", fig.width=10, fig.align="center"}
...
Pričakovano, so $\mu_j$ porazdelitve normalne, saj jih vzorčimo iz normalne porazdelitve. Podobno velja za $\sigma_j^2$ saj vzorčimo iz inverzne gamma porazdelitve. Te krivulje predstavljajo gostote za povprečje oz. za varianco ocen iz matematike posamezne šole.

Hiperparametri so nekoliko težji za razumeti, in nisem niti ravno prepričan če si jih pravilno interpretiram. Poglejmo si najprej porazdelitve hiperparametrov.
...
$\mu$ je lepo simetrično normalno porazdeljen, kar je lepo razvidno iz slike. $\mu$ predpostavljam da si lahko razlagamo kot povprečje vseh šol (oz. skupin).

$\sigma_0^2$ je tudi lepo porazdeljen, bi skoraj rekel da normalno, vendar glede na navodila vemo da je porazdeljen po gama porazdelitvi. Gama porazdelitev se pri dovolj velikih parametrih lahko aproksimira z normalno porazdelitvijo vendar to ni bistvo te naloge. $\sigma_0^2$ si predstavljam kot porazdelitev povprečij varianc skupin. Torej kot nekakšno skupno varianco vsem skupinam.

$\eta^2$ je že vizualno asimetrična, tudi smiselno saj izhaja iz inverzne gama porazdelitve. Razlagam si ga kot varianco povprečij skupin (oz. šol v našem primeru).

$\nu_0$ je diskretne oblike poleg tega pa še asimetričen. Razlagam si ga kot nekakšen povezovalni faktor, ki pove koliko informacij (zdi se mi da variance) se "deli" med skupinami.

Poglejmo si še $95\%$ intervale zaupanja za vse izbrane parametre in hiperparametre. Dodana je še mediana kot izbrana središčna vrednost zaradi asimetričnosti nekaterih porazdelitev, ter ti. MCSE, ki smo ga že izračunali višje.
...
kable(cbind(rbind(mu_tbl, sig_tbl, hyp_tbl), MCSE), 
      align="c",
      digits=2) %>%
    kable_styling(full_width = FALSE)
...
MCSE je, glede na vrednosti (hiper)parametrov, majhen. Mediane oz. variance se razlikujejo glede na skupine, kar je pričakovano.

# 4. Shrinkage

Podobno kot na vajah si poglejmo _shrinkage_ za povprečja.
...
smu1 <- ggplot(NULL, aes(x=pod.sole$povprecje, y=pod.sole$EmuGroups)) +
    geom_point() +
    geom_abline(intercept=0, slope=1) +
    labs(x=expression(bar(x)),
         y=expression(E(mu[j])))

ssig1 <- ggplot(NULL, aes(x=pod.sole$varianca, y=pod.sole$Esigma2Groups)) +
    geom_point() +
    geom_abline(intercept=0, slope=1) +
...
smu2 <- ggplot(NULL, aes(x=pod.sole$n, y=pod.sole$povprecje-pod.sole$EmuGroups)) +
    geom_point() +
    geom_hline(yintercept=0) +
    labs(x="Velikost vzorca šole",
         y=expression(bar(x)-E(mu[j])))

ssig2 <- ggplot(NULL, aes(x=pod.sole$n, y=pod.sole$varianca-pod.sole$Esigma2Groups)) +
    geom_point() +
    geom_hline(yintercept=0) +
    labs(x="Velikost vzorca šole",
...
Velikih razlik v primerjavi s prvotnim modelom pravzaprav ni videti. Je pa lepo videti _shrinkage_ efekt, ki pokaže kako skupine ki bolj odstopajo od povprečja, jih bolj "popravimo" s skupinskim povprečjem $\mu$. Podobno je videti na sosednji sliki kako skupine z manjšim številom zapisov bolj "popravimo" (ker jim manj verjamemo) medtem ko večje skupine pa manj.

Poglejmo si še podobne slike za variance skupin.

```{r, echo=FALSE, eval=TRUE, results="markup", fig.width=10, fig.align="center"}
grid.arrange(ssig1, ssig2, ncol=2)

```

Na prvi sliki je videti podobno stvar kot v prejšnjem primeru, da bolj "ekstremne" primere bolj "skrčimo" k pričakovani vrednosti skupne variance. Zdi se mi da velikost skupine v temu primeru veliko manj vpliva na to koliko "skrčimo" varianco, je pa to še vedno opaziti.

# 5. Primerjajava

Če primerjamo rezultate modela iz vaj in trenutnega modela, vidimo da pri rezultatih za povprečja ($\mu$ in $\eta^2$) večjih razlik pravzaprav ni. Torej ko smo dodali varianco za posamezne skupine, nismo veliko prispevali k tem porazdelitvam. Smo pa z upeljavo varianc posameznih šol pridobili dodatno informacijo o šoli, medtem ko smo prej predpostavili da so variance za vse šole enake. 

Model z različnimi variancami je, po mojem mnenju, smiselno uporabiti v primerih ko nas zanimajo tudi parametri o variancah. V temu primeru (o šolah), bi rekel da ne vidim razloga da ne bi uporabili modela z različnimi variancami. Računsko se mi ne zdi veliko bolj potraten, da bi lahko upravičili neuporabo modela z različnimi variancami. Če nas pa zanimajo samo povprečja, in če je tudi podatkov precej več, in je potrebnih več iteracij, je pa smiselno razmisliti o temu da uporabimo prvotno različico modela, saj ne bi storili velike napake pri izračunavanju parametrov za povprečja.

<!-- a obstaja kak način kako bi "zmanjšali" podatke, če so preveliki, da bi videli če z vpeljavo različnih varianc precej vplivamo na povprečja? vprašaj na vajah -->
