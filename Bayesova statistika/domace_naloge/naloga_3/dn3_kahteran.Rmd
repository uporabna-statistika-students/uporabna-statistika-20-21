...
pol podobno kot zgoraj sam prvih 500. 
...
pol tisti boxploti podvzorcev

```{r, echo=FALSE, eval=TRUE, results="markup", fig.align="center", fig.width=10}

hyper_data <- mutate(hyper_data, podvzorec=factor(rep(1:10, 4*n.iter/10)))
mu_data <- mutate(mu_data, podvzorec=factor(rep(1:10, 4*n.iter/10)))
sig_data <- mutate(sig_data, podvzorec=factor(rep(1:10, 4*n.iter/10)))
...
eff.smpl.data <- cbind(eff.smpl.param, eff.smpl.size)

colnames(eff.smpl.data) <- c("(hiper)parameter", "Effective sample size")

eff.smpl.data <- as_tibble(eff.smpl.data) %>% mutate_at("Effective sample size", as.double)

kable(eff.smpl.data, align="c", digits=2) %>%
    kable_styling(full_width=FALSE)

```

3. robne/marginalne aposteriorne porazdelitve in 95% intervali zaupanja in interpretacija rezultatov
