...
f.2 <- function(x, a=1000){
  pmin(x, a)
}

f.3 <- function(x, a=1000){
  x[x>a] <- a
  x
}
...
microbenchmark(f(x), f.2(x), f.3(x), times = 10000)
```
...
+ Poisci ze obstojece resitve: Google, StackExchange, StackOverflow, ipd. (naj bodo kredibilni)
+ Uporabi funkcijo, ki naredi cim manj: izracuna le to kar rabis, je prilagojena tvojemu tipu podatkov
+ Vektoriziraj: izogibaj se for zankam, uporabi funkcije, ki racunajo v C (npr. `colMeans`, ipd., `cumsum`, `diff`, itd.)
...
profvis({
  ...
  means1 <- apply(data1[, names(data1) != "id"], 2, mean)
  means2 <- colMeans(data1[, names(data1) != "id"])
  means3 <- lapply(data1[, names(data1) != "id"], mean)
  means4 <- vapply(data1[, names(data1) != "id"], mean, numeric(1))
})
``` 
...
Ce moramo izvesti neodvisne funkcije velikokrat, lahko paraleliziramo nase delo v R-u. Ogledali si bomo zelo preprost primer, ki uporabi funkcijo `lapply` in jo paralelizira. Opomba: ukazi in postopki so razlicni glede na operacijski sistem racunalnika.
